...
// Pulse Sensor
const int PULSE_SENSOR_PIN = 4; // Use um pino ADC adequado para o seu ESP32
PulseSensorPlayground pulseSensor;
const int THRESHOLD = 550; // Ajuste este valor para as leituras do seu sensor

int currentBPM = 0;
int currentSpO2 = 0;
unsigned long tempoDedoPresente = 0;
bool dedoPresente = false;
unsigned long tempoInicioDedo = 0;
...
void setup() {
  ...
  server.on("/getBPMData", handleGetBPMData);
  server.begin();
}
...
void loop() {
  ...
  if (pulseSensor.sawNewSample()) {
    if (pulseSensor.sawStartOfBeat()) {
      ...
      if (!dedoPresente) {
        tempoInicioDedo = millis();
        dedoPresente = true;
      }
      tempoDedoPresente = (millis() - tempoInicioDedo) / 1000;
      // Valor de espaço reservado para SpO2, pois o Pulse Sensor não o mede
      currentSpO2 = 98; // Placeholder value
      historico[indiceHistorico] = { currentBPM, currentSpO2, tempoDedoPresente };
      indiceHistorico = (indiceHistorico + 1) % MAX_DADOS;
    }
  }

  // A simple way to check if a finger is present is to check the signal quality.
  // If the signal is weak, we can assume the finger is not present.
  // This threshold might need adjustment depending on your sensor and environment.
  if (pulseSensor.getLatestSample() < THRESHOLD / 2 || pulseSensor.getBeatsPerMinute() < 30) { // Using a lower sample value or very low BPM as indicator
      currentBPM = 0;
      currentSpO2 = 0;
      dedoPresente = false;
      tempoDedoPresente = 0;
  ...
  delay(20); // Recommended delay by the PulseSensor Playground library
}
...
String gerarPagina(String titulo, String corpo) {
  return "<!DOCTYPE html><html><head><meta charset='UTF-8'><title>" + titulo + "</title><style>"
         ...
         // Function to generate PDF (specific to historicoUsuario page)
         "function generatePdf() {"
         "  const { jsPDF } = window.jspdf;"
         "  const doc = new jsPDF();"
         "  doc.setFontSize(18);"
         "  doc.text('Histórico de Batimentos', 14, 22);"
         "  doc.setFontSize(12);"
         "  doc.text('Nome: " + nome + "', 14, 30);" // Injecting profile data
         "  doc.text('Idade: " + idade + "', 14, 37);"
         "  doc.text('Email: " + email + "', 14, 44);"
         "  // AutoTable plugin for easy table generation"
         "  doc.autoTable({"
         "    html: '#historicoTable'," // ID of the table to convert"
         "    startY: 55," // Start below the header info"
         "    headStyles: { fillColor: [240, 98, 146] }," // Menu background color"
         "    altRowStyles: { fillColor: [248, 187, 208] }," // Card background color"
         "    styles: { cellPadding: 2, fontSize: 10, valign: 'middle', halign: 'center' },"
         "    columnStyles: { 0: { cellWidth: 30 }, 1: { cellWidth: 30 }, 2: { cellWidth: 30 } }"
         "  });"
         "  doc.save('historico_batimentos.pdf');"
         "}"
         "</script></head><body>"
         "<div class='menu'>"
         "<a href='/'>&#127968; Home</a>"
...
void handleHome() {
  ...
  corpo += "<div class='card'><h2>Gráficos em Tempo Real</h2>"
           ...
           "        timeData.push(data.time);" // Assuming 'time' is available in data
...
void handleHistoricoUsuario() {
  ...
  // Added id='historicoTable' to the table for jsPDF to find it
  corpo += "<div class='card'><h3>Histórico de Batimentos</h3><table id='historicoTable'><tr><th>BPM</th><th>SpO2</th><th>Tempo (s)</th></tr>";
  for (int i = 0; i < MAX_DADOS; i++) {
    if (historico[i].bpm > 0) {
      corpo += "<tr><td>" + String(historico[i].bpm) + "</td><td>" + String(historico[i].spo2) + "</td><td>" + String(historico[i].tempo) + "</td></tr>";
    }
  ...
  corpo += "<button onclick='generatePdf()'>Gerar PDF do Histórico</button>";
  corpo += "</div>"; // Close the card div
...
void handleGetBPMData() {
  ...
  json += "\"time\": " + String(tempoDedoPresente);
  json += "}";
